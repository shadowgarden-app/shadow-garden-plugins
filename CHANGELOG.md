...
- Marketplace renamed `shadow-garden` → `shadow-garden-skill-official`, the repository's name: Claude Desktop addresses a marketplace added from a repository by the repository name when it refreshes it, so a different `name` made its Update button fail (`NOT_REGISTERED`). Claude Code users re-add the marketplace and install `bapbong@shadow-garden-skill-official`.
